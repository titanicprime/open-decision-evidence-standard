...
Internal governance remains necessary, but it is insufficient when decision evidence must be consumed by another party that does not share the issuer's systems or internal controls. Cross-boundary reliance requires a transportable record of decision evidence, not merely confidence that an internal process existed.

This project addresses decision evidence portability rather than raw data sharing. The goal is to support portable evidence coordinates while preserving confidentiality, data locality, and implementation independence.
