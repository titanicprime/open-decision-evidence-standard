...
| `issuer.organization_id` | string | Yes | Identifies the issuing organization. | Required for minimally meaningful issuer identity. |
...
| `authority.authority_basis` | string | Yes | Describes the basis of authority. | Policy, delegation, contract, mandate, or similar. |
| `authority.authority_valid_at_decision` | boolean | Yes | Indicates whether authority was valid at decision time. | Useful for fail-closed verification. |
...
| `machine_role.ai_used` | boolean | Yes | Indicates whether AI or automation was used. | `false` is valid for non-AI decisions. |
| `machine_role.role` | enum string | Yes | Describes the machine role. | One of `none`, `retrieval`, `recommendation`, `assistance`, `execution`, `escalation`, `generation`, `unknown`. |
...
| `human_disposition.status` | enum string | Yes | States the human disposition. | Values include `accepted`, `modified`, `rejected`, `overridden`, `escalated`, `not_reviewed`, `reviewed_under_constraint`, `unknown`. |
| `human_disposition.review_substantiveness` | enum string | Yes | Describes depth of human review. | Distinguishes substantive from nominal or constrained review. |
| `human_disposition.machine_reliance_level` | enum string | Yes | Indicates degree of machine reliance. | Corresponds to the reliance-level concept. |
| `evidence` | object | Yes | Groups evidence commitment fields. | Core top-level object. |
| `evidence.evidence_commitment_type` | enum string | Yes | States how evidence is referenced or committed. | One of `hash`, `reference`, `attestation`, `proof`, `none`, `unknown`. |
...
| `evidence.selective_disclosure_available` | boolean | Yes | Indicates whether selective disclosure is supported. | Does not specify the mechanism. |
| `policy_basis` | array of objects | Yes | Lists policy or rule references. | Must contain at least one policy basis item in v0.1. |
| `policy_basis[].policy_id` | string | Yes | Identifies the policy, rule, or standard. | Required for each policy basis item. |
...
| `risk_coordinates.risk_tier` | enum string | Yes | States the risk tier. | One of `low`, `medium`, `high`, `critical`, `unknown`. |
| `risk_coordinates.jurisdiction` | string | Yes | Names the jurisdiction or context class. | Free-form in v0.1. |
...
| `status.superseded` | boolean | Yes | Indicates whether a newer record replaces this one. | When `true`, schema rules require `superseded_by`. |
| `status.revoked` | boolean | Yes | Indicates whether the record has been revoked. | When `true`, schema rules require `revoked_at`. |
...
| `verification.signature_type` | string | Yes | Names the signature or verification method. | May be `none` in minimal cases. |
| `verification.issuer_key_reference` | string | Yes | Points to the issuer key or key reference. | `none` may be used as an explicit placeholder in minimal records. |
| `verification.conformance_profile` | string | Yes | Names the conformance profile claimed. | Example: `pder_v0_1_minimal`. |
